...
interface Props {
  location: Location;
  renderLayOver: boolean;
  renderLoader: boolean;
  routeName: string;
  control: boolean;
}

export const Layout: React.FC<Props> = ({ location, renderLayOver, renderLoader, routeName, control }) => (
  <div className={'Layout Layout--' + routeName + (control ? ' Layout--control' : '')}>
    ...
    <div className="Layout-modalsAndPortals">
      <Fade mounted={renderLayOver} speed="fastest" position="fixed" appear>
        <div className="Layout-layOver">
          <Spinner className="Layout-layOverIcon" size="huge" />
        </div>
      </Fade>
